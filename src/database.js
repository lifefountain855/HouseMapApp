...
export const database = {
  /**
   * Fetch all house records — prefer per-user rows in Supabase when logged in.
   */
  async getHouses() {
    try {
      ...
      // console.log('[database.getHouses] getUser ->', { userId: user?.id, timestamp: new Date().toISOString() });

      if (user) {
        // Expect a table "houses" with columns: owner_id (UUID), data (jsonb)
        ...
        // console.log('[database.getHouses] supabase.select result ->', { data, error });

        if (error && error.code !== 'PGRST116') {
          // PGRST116 = no rows returned for single(); handle by creating row
          console.warn('Supabase select houses error:', error.message || error);
        }
        ...
        // No data yet for user — seed with mock data
        const { error: insertErr } = await supabase.from('houses').insert([{ owner_id: user.id, data: MOCK_HOUSES }]);
        if (insertErr) console.warn('Failed to seed houses for user', insertErr.message || insertErr);
        // console.log('[database.getHouses] seeded mock data for user', { userId: user.id });
        return MOCK_HOUSES;
      }

      // Fallback to AsyncStorage for unauthenticated users
      const local = await AsyncStorage.getItem(STORAGE_KEY);
      // console.log('[database.getHouses] unauthenticated fallback, local present?', local !== null);
      if (local !== null) {
      ...
      console.error('Failed to load houses from storage/supabase:', e);
      return [];
    }
  },
  ...
  async saveHouses(houses) {
    try {
      ...
      if (user) {
        // Upsert on user_id; ensure the Supabase table has a UNIQUE constraint on user_id
        const { error } = await supabase.from('houses').upsert({ owner_id: user.id, data: houses }, { onConflict: 'owner_id' });
        if (error) {
          console.warn('Supabase upsert houses error:', error.message || error);
          return false;
        }
        return true;
      }
      ...
      console.error('Failed to save houses to storage/supabase:', e);
      return false;
    }
  },
  ...
  async resetDatabase() {
    ...
    const user = userData?.user ?? null;
    if (user) {
      const { error } = await supabase.from('houses').upsert([{ owner_id: user.id, data: MOCK_HOUSES }], { onConflict: 'owner_id' });
      if (error) console.warn('Failed to reset houses for user', error.message || error);
      return MOCK_HOUSES;
    }
